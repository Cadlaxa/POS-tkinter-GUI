...
class POS(tk.Tk):
    ...
    def create_login_widgets(self):
        ...
        right_frame = ttk.Frame(main_frame, padding=10, style='Card.TFrame')
        right_frame.grid(row=0, column=1, sticky="nsew", padx=10)
        right_frame.grid_columnconfigure(0, weight=1)
        right_frame.grid_rowconfigure(0, weight=1)

        self.image = create_rounded_image(LOGIN_PIC, size=200, radius=30)
        image_label = ttk.Label(right_frame, image=self.image)
        image_label.grid(row=0, column=0, sticky="nsew", padx=10, pady=10)

        # Left column with login/signup
        left_frame = ttk.Frame(main_frame, padding=10, style='Card.TFrame')
        left_frame.grid(row=0, column=0, sticky="nsew", padx=10)
        left_frame.grid_columnconfigure(0, weight=1)

        label_username = ttk.Label(left_frame, text="Username")
        ...
        label_password = ttk.Label(left_frame, text="Password")
        ...
        button_login = ttk.Button(left_frame, text="Login", command=self.login)
        button_login.grid(row=4, column=0, pady=20, sticky="ew")

        button_signup = ttk.Button(left_frame, text="Signup", command=self.signup)
